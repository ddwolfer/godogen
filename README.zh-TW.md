...
- **證明而不是宣稱** —— 從跑起來的遊戲判斷,絕不從編譯乾淨判斷。五層驗收,按「最常抓到真問題」排序:你實際玩、**沒有前後文的另一個 agent** 讀截圖、量測探針、自動化測試、死鎖檢查。讀截圖那層刻意換人,因為蓋的人知道火球在那裡,所以他就看見了。
...
- **已經做出過兩款,而且兩款都是塔防** ——《元素牌堆》(元素是一副抽的牌)與《血脈防線》(種族專精 + 遺物三選一)。語料裡有 8 條來自它們。缺的不是第二款,是**第二種類型**:在那之前,沒有辦法分辨那些條目哪些是關於遊戲、哪些只是關於塔防。
